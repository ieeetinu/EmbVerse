/*******************************************************************************
* File Name: Modbus_RTU_Over_UART_IntClock.c
* Version 2.20
*
*  Description:
...
/*******************************************************************************
* Function Name: Modbus_RTU_Over_UART_IntClock_Start
********************************************************************************
*
* Summary:
...
void Modbus_RTU_Over_UART_IntClock_Start(void) 
{
    ...
    Modbus_RTU_Over_UART_IntClock_CLKEN |= Modbus_RTU_Over_UART_IntClock_CLKEN_MASK;
	Modbus_RTU_Over_UART_IntClock_CLKSTBY |= Modbus_RTU_Over_UART_IntClock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Modbus_RTU_Over_UART_IntClock_Stop
********************************************************************************
*
* Summary:
...
void Modbus_RTU_Over_UART_IntClock_Stop(void) 
{
    ...
    Modbus_RTU_Over_UART_IntClock_CLKEN &= (uint8)(~Modbus_RTU_Over_UART_IntClock_CLKEN_MASK);
	Modbus_RTU_Over_UART_IntClock_CLKSTBY &= (uint8)(~Modbus_RTU_Over_UART_IntClock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Modbus_RTU_Over_UART_IntClock_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Modbus_RTU_Over_UART_IntClock_CLKEN & Modbus_RTU_Over_UART_IntClock_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Modbus_RTU_Over_UART_IntClock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Modbus_RTU_Over_UART_IntClock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Modbus_RTU_Over_UART_IntClock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Modbus_RTU_Over_UART_IntClock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Modbus_RTU_Over_UART_IntClock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Modbus_RTU_Over_UART_IntClock_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Modbus_RTU_Over_UART_IntClock_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Modbus_RTU_Over_UART_IntClock_GetSourceRegister();
    uint16 oldDivider = Modbus_RTU_Over_UART_IntClock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Modbus_RTU_Over_UART_IntClock_CLKEN & Modbus_RTU_Over_UART_IntClock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(Modbus_RTU_Over_UART_IntClock_DIV_PTR, clkDivider);
                Modbus_RTU_Over_UART_IntClock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                Modbus_RTU_Over_UART_IntClock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(Modbus_RTU_Over_UART_IntClock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Modbus_RTU_Over_UART_IntClock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(Modbus_RTU_Over_UART_IntClock_DIV_PTR, clkDivider);
				Modbus_RTU_Over_UART_IntClock_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: Modbus_RTU_Over_UART_IntClock_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Modbus_RTU_Over_UART_IntClock_DIV_PTR);
}


/*******************************************************************************
* Function Name: Modbus_RTU_Over_UART_IntClock_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Modbus_RTU_Over_UART_IntClock_MOD_SRC |= modeBitMask & (uint8)Modbus_RTU_Over_UART_IntClock_MODE_MASK;
}


/*******************************************************************************
* Function Name: Modbus_RTU_Over_UART_IntClock_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Modbus_RTU_Over_UART_IntClock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Modbus_RTU_Over_UART_IntClock_MODE_MASK));
}


/*******************************************************************************
* Function Name: Modbus_RTU_Over_UART_IntClock_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Modbus_RTU_Over_UART_IntClock_MOD_SRC & (uint8)(Modbus_RTU_Over_UART_IntClock_MODE_MASK);
}


/*******************************************************************************
* Function Name: Modbus_RTU_Over_UART_IntClock_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Modbus_RTU_Over_UART_IntClock_GetDividerRegister();
    uint8 oldSrc = Modbus_RTU_Over_UART_IntClock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Modbus_RTU_Over_UART_IntClock_MOD_SRC |= CYCLK_SSS;
        Modbus_RTU_Over_UART_IntClock_MOD_SRC =
            (Modbus_RTU_Over_UART_IntClock_MOD_SRC & (uint8)(~Modbus_RTU_Over_UART_IntClock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Modbus_RTU_Over_UART_IntClock_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Modbus_RTU_Over_UART_IntClock_MOD_SRC & Modbus_RTU_Over_UART_IntClock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Modbus_RTU_Over_UART_IntClock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Modbus_RTU_Over_UART_IntClock_PHASE = clkPhase & Modbus_RTU_Over_UART_IntClock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Modbus_RTU_Over_UART_IntClock_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Modbus_RTU_Over_UART_IntClock_PHASE & Modbus_RTU_Over_UART_IntClock_PHASE_MASK;
}
